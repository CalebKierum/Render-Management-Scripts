    ...
    function SlickGrid(container, data, columns, options) {
        ...
        function scrollTo(y) {
            ...
            if (prevScrollTop != newScrollTop) {
                ...
                setScrollTop(lastRenderedScrollTop = scrollTop = prevScrollTop = newScrollTop);
        ...
        function updateRowCount() {
            ...
            // TV: add scrollbar height to canvas so that bottom rows don't get obscured by horizontal scrollbar
            //th = Math.max(options.rowHeight * newRowCount, viewportH - scrollbarDimensions.height);
            th = Math.max(options.rowHeight * newRowCount + scrollbarDimensions.height, viewportH - scrollbarDimensions.height);
        ...
        function getScrollTop() {
            // if jScrollPane is used, need to deteremine scroll offset through API
            var jsp = $viewport.data("jsp");
            if (jsp) {
                return jsp.getContentPositionY();
            } else {
        ...
        function getScrollLeft() {
            ...
            var jsp = $viewport.data("jsp");
            if (jsp) {
                return jsp.getContentPositionX();
            } else {
                return $viewport[0].scrollLeft;
            }
        }
                ...
                if (!e.shiftKey && !e.altKey && !e.ctrlKey) {
                    if (e.which == 27) {
                        ...
                        if (options.editable) {
                            ...
                            return; // otherwise event will not be propogated -- TV
                        }
                    } else
                        return;
        ...
        function handleClick(e) {
            ...
            if (item && (c.behavior === "selectAndMove" || c.behavior === "select" || (e.ctrlKey || e.shiftKey || e.altKey))) {
                ...
                if (validated) {
                    ...
                    if (!e.ctrlKey && !e.shiftKey && !e.metaKey && !e.altKey) {
                        ...
                        if (idx === -1 && (e.ctrlKey || e.metaKey || e.altKey)) {
                            selection.push(row);
                        } else if (idx !== -1 && (e.ctrlKey || e.metaKey || e.altKey)) {
        ...
        function focusOnCurrentCell() {
            ...
            // TODO: may need to use get/setScrollLeft() functions if you end up using cell navigation
