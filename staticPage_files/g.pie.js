...
(function () {
    ...
    function Piechart(paper, cx, cy, r, someValues, opts) {
        opts = opts || {};
        var VALUE_EPSILON = 0.000001;
        var values = [];
        for (var i=0; i < someValues.length; i++) {
            values.push(someValues[i] ? someValues[i] : VALUE_EPSILON);
        }
        var chartinst = this,
            sectors = [],
            covers = paper.set(),
            ...
            minPercent = typeof(opts.minPercent) === "number" ? parseFloat(opts.minPercent) : 1,
